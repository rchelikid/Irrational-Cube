...
//This is where Kyle added the cubli mixer
static const motorMixer_t mixerCubli[] = { //n, throttle, roll, pitch, yaw
    { 1.0f, 0.0f, 0.0f,  0.0f },     // ROLL
...
const mixer_t mixers[] = {
    ...
    { 3, false, mixerQuadP },          // MIXER_QUADP
    ...
    { 3, false, mixerCubli },           // MIXER_CUBLI
};
...
// this is only for custom motor mixers
...
void calculateCurrentThrottle(void) // throttle
{
    ...
    if (featureIsEnabled(FEATURE_3D)) {
        ...
        if (IS_RC_MODE_ACTIVE(BOX3D) || flight3DConfig()->switched_mode3d) {
            ...
            rcCommand3dDeadBandLow = rxConfig()->midrc - flight3DConfig()->deadband3d_throttle; // 1450
            rcCommand3dDeadBandHigh = rxConfig()->midrc + flight3DConfig()->deadband3d_throttle; // 1550
        }

        const float rcCommandThrottleRange3dLow = rcCommand3dDeadBandLow - PWM_RANGE_MIN; // positive
        const float rcCommandThrottleRange3dHigh = PWM_RANGE_MAX - rcCommand3dDeadBandHigh; // positive

        if (rcCommand[THROTTLE] <= rcCommand3dDeadBandLow || isFlipOverAfterCrashActive()) {
            // INVERTED

            rcThrottlePrevious = rcCommand[THROTTLE]; // low
            throttle = rcCommand[THROTTLE] - rcCommand3dDeadBandLow; // negative
            currentThrottleInputRange = rcCommandThrottleRange3dLow; // positive
        } else if (rcCommand[THROTTLE] >= rcCommand3dDeadBandHigh) {
            // NORMAL

            rcThrottlePrevious = rcCommand[THROTTLE]; // high
            throttle = rcCommand[THROTTLE] - rcCommand3dDeadBandHigh;// positive
            currentThrottleInputRange = rcCommandThrottleRange3dHigh;// positive
        ...
        throttle = constrainf(throttle / currentThrottleInputRange, -1.0f, 1.0f);
    } else {
...
// end calculate throttle

// calculate end points
float calculateCurrentMotorEndpoints(float motorSetpoint) // end points
{
    static uint16_t rcThrottlePrevious = 0;   // Store the last throttle direction for deadband transitions
    ...
    } else { // this is cubli now
        motorOutputMixSign = 1;
        if (motorSetpoint < -motorDeadbandCubli) { // disable isFlipOverAfterCrashActive() for cubli
            // INVERTED
            motorRangeMin = motorOutputLow;
            motorRangeMax = deadbandMotor3dLow;

            motorOutputMin = motorRangeMin;
            motorOutputRange = motorRangeMin - motorRangeMax; // negative

            motorSetpoint = motorSetpoint + motorDeadbandCubli;
        } else if (motorSetpoint > motorDeadbandCubli) {
            // NORMAL
            motorRangeMin = deadbandMotor3dHigh;
            motorRangeMax = motorOutputHigh;

            motorOutputMin = motorRangeMin;
            motorOutputRange = motorRangeMax - motorRangeMin; // (positive)

            motorSetpoint = motorSetpoint - motorDeadbandCubli;
        } else {
            // IN DEADBAND
            motorRangeMin = disarmMotorOutput; // stop motor
            motorRangeMax = disarmMotorOutput; // stop motor
            motorOutputMin = disarmMotorOutput;
            motorOutputRange = disarmMotorOutput;
        }
    }
  } else { // this is not 3D
...
// #endif
        cubliBalanceStart();
        if (featureIsEnabled(FEATURE_CUBLI)) {
        motorOutput = (calculateCurrentMotorEndpoints(motorOutput)) * motorOutputRange;// doesn't add idle
        } else {
        motorOutput += motorOutputThrottle;
        motorOutput = motorOutputMin + (calculateCurrentMotorEndpoints(motorOutput) * motorOutputRange); // just throttle
        }
...
#ifdef USE_DSHOT
            if (isMotorProtocolDshot()) {
                motorOutput = (motorOutput < motorOutputLow) ? disarmMotorOutput : motorOutput; // Prevent getting into special reserved range
            }
#endif
            motorOutput = constrain(motorOutput, disarmMotorOutput, motorRangeMax);
        } else if (featureIsEnabled(FEATURE_CUBLI)) {
        motorOutput = motor[i] + ((motorGain / 1000.0f) * motorOutput * targetPidLooptime * 1e-6f);
...
FAST_CODE_NOINLINE void mixTable(timeUs_t currentTimeUs, uint8_t vbatPidCompensation)
{
    ...
    motorMixRange = motorMixMax - motorMixMin;
     if (motorMixRange > 1.0f) {
         for (int i = 0; i < motorCount; i++) { // makes motor range less than 1
